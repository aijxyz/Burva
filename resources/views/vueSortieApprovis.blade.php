...
<select style="width: 200px;" name="toggle_column"  id="toggle_column">
  ...
  <div class="col-md-12" style="margin-left:30px; margin-right:50px" >                   
        ...
        <div class="row mt-5">         
            ...
            <h6 class="card-title">Sortie Approvisionement
              ...
              <div class="table-responsive p-0">
                <table id="id_sortieAppro" class="table table-bordered table-striped  display ">
                  <thead>
                    <tr> 
                  ...
                  <tbody>
                   @foreach($sortieApprovis as $sortieApproviscat)
                  <tr>   
                    <td class="text-center">{{$sortieApproviscat->id}}</td>               
                    <td class="text-center">{{$sortieApproviscat->debutSerie}}</td>                        
                    <td class="text-center">{{$sortieApproviscat->finSerie}}</td> 
                    <td class="text-center">{{$sortieApproviscat->dateSortie}}</td>  
                    <td class="text-center">{{$sortieApproviscat->centre}}</td> 
                    <td class="text-center">{{$sortieApproviscat->prix}}</td>                   
                    <td class="text-center">{{$sortieApproviscat->entreApprovis->dateEntre}}</td>
                    <td class="text-center">{{$sortieApproviscat->prixTotal}}</td>
                    <td class="text-center"> 
                        <button class="btn btn-info" data-toggle="modal"
                         data-mydebut="{{$sortieApproviscat->debutSerie}}"
                         data-myfin="{{$sortieApproviscat->finSerie}}"
                         data-mydate="{{$sortieApproviscat->dateSortie}}"
                         data-mycentre="{{$sortieApproviscat->centre}}"
                         data-myprix="{{$sortieApproviscat->prix}}"
                         data-myprixtotal="{{$sortieApproviscat->prixTotal}}"
                         data-myidentreappro="{{$sortieApproviscat->entreApprovis->id}}"                         
                         data-catid="{{$sortieApproviscat->id}}"
                         data-target="#modifier">
                         <i class="fa fa-edit"></i>
                        </button>
                         |                   
                        <button class="btn btn-danger" data-toggle="modal" data-target="#supp" data-catid="{{$sortieApproviscat->id}}">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body">
         ...
         <form id="myForm" method="POST" action="{{route('sortieApprovisAjout')}}">
              ...
              @csrf
                ...
                <div class="form-group row">          
                  <label for="id_entreAppro" class="col-md-4 col-form-label text-md-right">{{ __('Date entrée') }}</label>
                    <div class="col-md-6 "> 

                    <select id="id_entreAppro" onfocus='this.size=3;' onblur='this.size=1;' onchange='this.size=1; this.blur();'  type="text" class="form-control" name="entreApprovis_id"  required autocomplete="id_entreAppro" autofocus>                     
                    <option> </option>  
                     @foreach($entreAppro as $entreAppros)

                     <option value="{{$entreAppros->id}}">{{$entreAppros->dateEntre}}</option>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body">
         ...
         <form id="myFormModif" method="POST" action="{{route('sortieApprovisModifier')}}">
              {{method_field('patch')}} 
              {{csrf_field()}} 
              <div class="form-group row">                  
                ...
                <div class="form-group row">          
                 ...
                 <div class="form-group row">          
                  <label for="id_entrebord" class="col-md-4 col-form-label text-md-right">{{ __('Date entrée') }}</label>
                    ...
                    <select id="id_entreAppro" onfocus='this.size=3;' onblur='this.size=1;' onchange='this.size=1; this.blur();'  type="text" class="form-control" name="entreAppros_id"  required autocomplete="id_entreAppro" autofocus>                     
                    <option> </option>  
                     @foreach($entreAppro as $entreAppros)
            ...
            <input id="cat_id" type="hidden" name="sortieApprovis_id" value=" " >  
              </div> 
...
<div class="modal fade" id="supp" tabindex="-1" role="dialog" aria-labelledby="supplLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-body">
     <form method="POST" action="{{ route('sortieApprovisSupp') }}">  
        {{method_field('delete')}}
        {{csrf_field()}}                             
             <div class="modal-body"> 

                <h2 class="text-center" style="color: red">Voulez vous vraiment supprimer</br> cet Approvisionement?&hellip;</h2>
...
<!--<script src="https://unpkg.com/multiple-select@1.3.1/dist/multiple-select.js"></script>-->
   ...
   // $(document).ready(function () {
    ...
    var table = $('#id_sortieAppro').DataTable({
   ...
   var y = document.getElementById("myFormModif");    
    ...
    $('#modifier').on('show.bs.modal',function(event){
          // console.log('test ooooooo');
           ...
           var cat_id=button.data('catid')
           var id_entreAppro=button.data('myidentreappro')                 
           ...
           modal.find('.modal-body #id_entreAppro').val(id_entreAppro);     
           modal.find('.modal-body #cat_id').val(cat_id);
