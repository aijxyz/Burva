   ...
   <div class="row mt-5">
    ...
    <div class="card">
      <div class="card-header">
        <h6 class="card-title">Entrée fiche de  Maintenance DAB
      ...
      <div class="table-responsive p-0">
        <table id="id_entremain" class="table table-bordered table-striped  display ">
          <thead>
            <tr> 
          ...
          <tbody>
           @foreach($entreMaintenances as $entreMaintenancescat)
           <tr>   
             <td class="text-center">{{$entreMaintenancescat->id}}</td>              
             <td class="text-center">{{$entreMaintenancescat->debutSerie}}</td>                        
             <td class="text-center">{{$entreMaintenancescat->finSerie}}</td>     
             <td class="text-center">{{$entreMaintenancescat->dateEntre}}</td>  
             <td class="text-center">{{$entreMaintenancescat->prixUnitaire}}</td> 
             <td class="text-center">{{$entreMaintenancescat->fournisseur->nom}}</td>
             <td class="text-center">{{$entreMaintenancescat->prixTotal}}</td>                     
             <td class="text-center"> 
              <button class="btn btn-info" data-toggle="modal"
              data-mydebut="{{$entreMaintenancescat->debutSerie}}"
              data-myfin="{{$entreMaintenancescat->finSerie}}"
              data-mydate="{{$entreMaintenancescat->dateEntre}}"
              data-myprix="{{$entreMaintenancescat->prixUnitaire}}"
              data-myprixtotal="{{$entreMaintenancescat->prixTotal}}"
              data-myidfour="{{$entreMaintenancescat->fournisseur->id}}"                         
              data-catid="{{$entreMaintenancescat->id}}"
              data-target="#modifier">
              <i class="fa fa-edit"></i>
            </button>
            |                   
            <button class="btn btn-danger" data-toggle="modal" data-target="#supp" data-catid="{{$entreMaintenancescat->id}}">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form id="myForm" method="POST" action="{{route('entreMaintenanceAjout')}}">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form id="myFormModif" method="POST" action="{{route('entreMaintenanceModifier')}}">
        {{method_field('patch')}} 
        {{csrf_field()}} 
        <div class="form-group row">                  
...
<input id="cat_id" type="hidden" name="entreMaintenance_id" value=" " >  
</div> 
...
<div class="modal fade" id="supp" tabindex="-1" role="dialog" aria-labelledby="supplLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       <form method="POST" action="{{ route('entreMaintenanceSupp') }}">  
        {{method_field('delete')}}
        {{csrf_field()}}                             
        <div class="modal-body"> 
    ...
    $(document).ready(function(){ 
     ...
     /* var table = jQuery('#id_four').DataTable( {
      ...
      //id_four = jQuery('#id_four').DataTable();
          ...
          var table = $('#id_entremain').DataTable( {
