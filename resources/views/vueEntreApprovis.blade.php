   ...
   <div class="row mt-5">
    ...
    <div class="card">
      <div class="card-header">
        <h6 class="card-title">Entrée Approvisionement
      ...
      <div class="table-responsive p-0">
        <table id="id_entreAppro" class="table table-bordered table-striped  display ">
          <thead>
            <tr> 
          ...
          <tbody>
           @foreach($entreApprovis as $entreApproviscat)
           <tr>   
             <td class="text-center">{{$entreApproviscat->id}}</td>              
             <td class="text-center">{{$entreApproviscat->debutSerie}}</td>                        
             <td class="text-center">{{$entreApproviscat->finSerie}}</td>     
             <td class="text-center">{{$entreApproviscat->dateEntre}}</td>  
             <td class="text-center">{{$entreApproviscat->prixUnitaire}}</td> 
             <td class="text-center">{{$entreApproviscat->fournisseur->nom}}</td>
             <td class="text-center">{{$entreApproviscat->prixTotal}}</td>                     
             <td class="text-center"> 
              <button class="btn btn-info" data-toggle="modal"
              data-mydebut="{{$entreApproviscat->debutSerie}}"
              data-myfin="{{$entreApproviscat->finSerie}}"
              data-mydate="{{$entreApproviscat->dateEntre}}"
              data-myprix="{{$entreApproviscat->prixUnitaire}}"
              data-myprixtotal="{{$entreApproviscat->prixTotal}}"
              data-myidfour="{{$entreApproviscat->fournisseur->id}}"                         
              data-catid="{{$entreApproviscat->id}}"
              data-target="#modifier">
              <i class="fa fa-edit"></i>
            </button>
            |                   
            <button class="btn btn-danger" data-toggle="modal" data-target="#supp" data-catid="{{$entreApproviscat->id}}">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form id="myForm" method="POST" action="{{route('entreApprovisAjout')}}">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form id="myFormModif" method="POST" action="{{route('entreApprovisModifier')}}">
        {{method_field('patch')}} 
        {{csrf_field()}} 
        <div class="form-group row">                  
...
<input id="cat_id" type="hidden" name="entreApprovis_id" value=" " > 
</div> 
...
<div class="modal fade" id="supp" tabindex="-1" role="dialog" aria-labelledby="supplLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       <form method="POST" action="{{ route('entreApprovisSupp') }}">  
        {{method_field('delete')}}
        {{csrf_field()}}                             
        <div class="modal-body"> 
    ...
    $(document).ready(function(){ 
     ...
     /* var table = jQuery('#id_four').DataTable( {
      ...
      //id_four = jQuery('#id_four').DataTable();
          ...
          var table = $('#id_entreAppro').DataTable( {
