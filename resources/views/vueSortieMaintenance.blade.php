...
<select style="width: 200px;" name="toggle_column"  id="toggle_column">
  ...
  <div class="col-md-12" style="margin-left:30px; margin-right:50px" >                   
        ...
        <div class="row mt-5">         
            ...
            <h6 class="card-title">Sortie fiche Maintenance DAB
              ...
              <div class="table-responsive p-0">
                <table id="id_sortieMaint" class="table table-bordered table-striped  display ">
                  <thead>
                    <tr> 
                  ...
                  <tbody>
                   @foreach($sortieMaintenances as $sortieMaintenancescat)
                  <tr>   
                    <td class="text-center">{{$sortieMaintenancescat->id}}</td>               
                    <td class="text-center">{{$sortieMaintenancescat->debutSerie}}</td>                        
                    <td class="text-center">{{$sortieMaintenancescat->finSerie}}</td> 
                    <td class="text-center">{{$sortieMaintenancescat->dateSortie}}</td>  
                    <td class="text-center">{{$sortieMaintenancescat->centre}}</td> 
                    <td class="text-center">{{$sortieMaintenancescat->prix}}</td>                   
                    <td class="text-center">{{$sortieMaintenancescat->entreMaintenance->dateEntre}}</td>
                    <td class="text-center">{{$sortieMaintenancescat->prixTotal}}</td>
                    <td class="text-center"> 
                        <button class="btn btn-info" data-toggle="modal"
                         data-mydebut="{{$sortieMaintenancescat->debutSerie}}"
                         data-myfin="{{$sortieMaintenancescat->finSerie}}"
                         data-mydate="{{$sortieMaintenancescat->dateSortie}}"
                         data-mycentre="{{$sortieMaintenancescat->centre}}"
                         data-myprix="{{$sortieMaintenancescat->prix}}"
                         data-myprixtotal="{{$sortieMaintenancescat->prixTotal}}"
                         data-myidentremaint="{{$sortieMaintenancescat->entreMaintenance->id}}"                         
                         data-catid="{{$sortieMaintenancescat->id}}"
                         data-target="#modifier">
                         <i class="fa fa-edit"></i>
                        </button>
                         |                   
                        <button class="btn btn-danger" data-toggle="modal" data-target="#supp" data-catid="{{$sortieMaintenancescat->id}}">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body">
         ...
         <form id="myForm" method="POST" action="{{route('sortieMaintenanceAjout')}}">
              ...
              @csrf
                ...
                <div class="form-group row">          
                  <label for="id_entreMaint" class="col-md-4 col-form-label text-md-right">{{ __('Date entrée') }}</label>
                    <div class="col-md-6 "> 

                    <select id="id_entreMaint" onfocus='this.size=3;' onblur='this.size=1;' onchange='this.size=1; this.blur();'  type="text" class="form-control" name="entreMaintenance_id"  required autocomplete="id_entreMaint" autofocus>                     
                    <option> </option>  
                     @foreach($entreMaint as $entreMaints)

                     <option value="{{$entreMaints->id}}">{{$entreMaints->dateEntre}}</option>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body">
         ...
         <form id="myFormModif" method="POST" action="{{route('sortieMaintenanceModifier')}}">
              {{method_field('patch')}} 
              {{csrf_field()}} 
              <div class="form-group row">                  
                ...
                <div class="form-group row">          
                  <label for="id_entreMaint" class="col-md-4 col-form-label text-md-right">{{ __('Date Entrée') }}</label>
                    ...
                    <select id="id_entreMaint" onfocus='this.size=3;' onblur='this.size=1;' onchange='this.size=1; this.blur();'  type="text" class="form-control" name="entreBordereau_id"  required autocomplete="id_entreMaint" autofocus>                     
            ...
            <input id="cat_id" type="hidden" name="sortieMaintenance_id" value=" " >  
              </div> 
...
<div class="modal fade" id="supp" tabindex="-1" role="dialog" aria-labelledby="supplLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-body">
     <form method="POST" action="{{ route('sortieMaintenanceSupp') }}">  
        {{method_field('delete')}}
        {{csrf_field()}}                             
             <div class="modal-body"> 

              <h2 class="text-center" style="color: red">Voulez vous vraiment supprimer</br> cette fiche de maintenance DAB?&hellip;</h2>
              </div>                             
       <input id="cat_id" type="hidden" name="sortieMaintenance_id" value=" " >       
      <div class="modal-footer">
...
<!--<script src="https://unpkg.com/multiple-select@1.3.1/dist/multiple-select.js"></script>-->
   ...
   // $(document).ready(function () {
    ...
    var table = $('#id_sortieMaint').DataTable({
   ...
   var y = document.getElementById("myFormModif");    
    ...
    $('#modifier').on('show.bs.modal',function(event){
          // console.log('test ooooooo');
           ...
           var cat_id=button.data('catid')
           var id_entreMaint=button.data('myidentremaint')                 
           ...
           modal.find('.modal-body #id_entreMaint').val(id_entreMaint);     
           modal.find('.modal-body #cat_id').val(cat_id);
