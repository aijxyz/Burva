...
<select style="width: 200px;" name="toggle_column"  id="toggle_column">
  ...
  <div class="col-md-12" style="margin-left:20px; margin-right:50px" >                   
        ...
        <div class="row mt-5">         
            ...
            <h6 class="card-title">Sortie Ticket visiteur
              ...
              <div class="table-responsive p-0">
                <table id="id_sortieTick" class="table table-bordered table-striped  display ">
                  <thead>
                    <tr> 
                  ...
                  <tbody>
                   @foreach($sortieTickets as $sortieTicketscat)
                  <tr>   
                    <td class="text-center">{{$sortieTicketscat->id}}</td>               
                    <td class="text-center">{{$sortieTicketscat->debutSerie}}</td>                        
                    <td class="text-center">{{$sortieTicketscat->finSerie}}</td> 
                    <td class="text-center">{{$sortieTicketscat->dateSortie}}</td>  
                    <td class="text-center">{{$sortieTicketscat->centre}}</td> 
                    <td class="text-center">{{$sortieTicketscat->prix}}</td>                   
                    <td class="text-center">{{$sortieTicketscat->entreTicket->dateEntre}}</td>
                    <td class="text-center">{{$sortieTicketscat->prixTotal}}</td>
                    <td class="text-center"> 
                        <button class="btn btn-info" data-toggle="modal"
                         data-mydebut="{{$sortieTicketscat->debutSerie}}"
                         data-myfin="{{$sortieTicketscat->finSerie}}"
                         data-mydate="{{$sortieTicketscat->dateSortie}}"
                         data-mycentre="{{$sortieTicketscat->centre}}"
                         data-myprix="{{$sortieTicketscat->prix}}"
                         data-myprixtotal="{{$sortieTicketscat->prixTotal}}"
                         data-myidentretick="{{$sortieTicketscat->entreTicket->id}}"                         
                         data-catid="{{$sortieTicketscat->id}}"
                         data-target="#modifier">
                         <i class="fa fa-edit"></i>
                        </button>
                         |                   
                        <button class="btn btn-danger" data-toggle="modal" data-target="#supp" data-catid="{{$sortieTicketscat->id}}">
             ...
             <tfoot>
                ...
                <div class="form-group row">
                  ...
                  <div class="col-md-3"></div>
                  <div class="col-md-3"></div>              
                  <div class="col-md-3 border border-dark"><strong>Total sortie:{{$som}}  </strong>FCFA</div>
                </div>
              <!-- /.card-body -->
            </div>
            <!-- /.card -->
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body">
         ...
         <form id="myForm" method="POST" action="{{route('sortieTicketAjout')}}">
                ...
                <div class="form-group row">          
                  <label for="id_entreTick" class="col-md-4 col-form-label text-md-right">{{ __('Date entrée') }}</label>
                    ...
                    <option> </option>  
                     ...
                     <option value="{{$entreTicks->id}}">{{$entreTicks->dateEntre}}</option>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body">
         ...
         <form id="myFormModif" method="POST" action="{{route('sortieTicketModifier')}}">
              {{method_field('patch')}} 
              {{csrf_field()}} 
              <div class="form-group row">                  
            ...
            <input id="cat_id" type="hidden" name="sortieTicket_id" value=" " >  
              </div> 
...
<div class="modal fade" id="supp" tabindex="-1" role="dialog" aria-labelledby="supplLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-body">
     <form method="POST" action="{{ route('sortieTicketSupp') }}">  
        {{method_field('delete')}}
        {{csrf_field()}}                             
             <div class="modal-body"> 

                <h2 class="text-center" style="color: red">Voulez vous vraiment supprimer</br> ce ticket visiteur?&hellip;</h2>
...
<!--<script src="https://unpkg.com/multiple-select@1.3.1/dist/multiple-select.js"></script>-->
   ...
   // $(document).ready(function () {
    ...
    var table = $('#id_sortieTick').DataTable({
   ...
   var y = document.getElementById("myFormModif");    
    ...
    $('#modifier').on('show.bs.modal',function(event){
          // console.log('test ooooooo');
           ...
           var cat_id=button.data('catid')
           var id_entreTick=button.data('myidentretick')                 
           ...
           modal.find('.modal-body #id_entreTick').val(id_entreTick);     
           modal.find('.modal-body #cat_id').val(cat_id);
