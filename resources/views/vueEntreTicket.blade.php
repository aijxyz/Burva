   ...
   <div class="row mt-5">
    ...
    <div class="card">
      <div class="card-header">
        <h6 class="card-title">Entré ticket visiteur
      ...
      <div class="table-responsive p-0">
        <table id="id_entreTick" class="table table-bordered table-striped  display ">
          <thead>
            <tr> 
          ...
          <tbody>
           @foreach($entreTickets as $entreTicketscat)
           <tr>   
             <td class="text-center">{{$entreTicketscat->id}}</td>              
             <td class="text-center">{{$entreTicketscat->debutSerie}}</td>                        
             <td class="text-center">{{$entreTicketscat->finSerie}}</td>     
             <td class="text-center">{{$entreTicketscat->dateEntre}}</td>  
             <td class="text-center">{{$entreTicketscat->prixUnitaire}}</td> 
             <td class="text-center">{{$entreTicketscat->fournisseur->nom}}</td>
             <td class="text-center">{{$entreTicketscat->prixTotal}}</td>                     
             <td class="text-center"> 
              <button class="btn btn-info" data-toggle="modal"
              data-mydebut="{{$entreTicketscat->debutSerie}}"
              data-myfin="{{$entreTicketscat->finSerie}}"
              data-mydate="{{$entreTicketscat->dateEntre}}"
              data-myprix="{{$entreTicketscat->prixUnitaire}}"
              data-myprixtotal="{{$entreTicketscat->prixTotal}}"
              data-myidfour="{{$entreTicketscat->fournisseur->id}}"                         
              data-catid="{{$entreTicketscat->id}}"
              data-target="#modifier">
              <i class="fa fa-edit"></i>
            </button>
            |                   
            <button class="btn btn-danger" data-toggle="modal" data-target="#supp" data-catid="{{$entreTicketscat->id}}">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form id="myForm" method="POST" action="{{route('entreTicketAjout')}}">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form id="myFormModif" method="POST" action="{{route('entreTicketModifier')}}">
        {{method_field('patch')}} 
        {{csrf_field()}} 
        <div class="form-group row">                  
...
<input id="cat_id" type="hidden" name="entreTicket_id" value=" " >  
</div> 
...
<div class="modal fade" id="supp" tabindex="-1" role="dialog" aria-labelledby="supplLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
       <form method="POST" action="{{ route('entreTicketSupp') }}">  
        {{method_field('delete')}}
        {{csrf_field()}}                             
        <div class="modal-body"> 
    ...
    $(document).ready(function(){ 
     ...
     /* var table = jQuery('#id_four').DataTable( {
      ...
      //id_four = jQuery('#id_four').DataTable();
          ...
          var table = $('#id_entreTick').DataTable( {
